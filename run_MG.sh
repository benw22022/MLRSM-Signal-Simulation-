...
cd /mnt/e/MG5_aMC_v2_6_7
hepmc_path="/mnt/e/PhD/Work/Majorana/signal_simulation/hepmc_samples"
for d in simulation_files/*
do  
    echo "d = " $d
    (cd "$d"/Events/run_01 && gunzip -f tag_1_pythia8_events.hepmc.gz || echo "Command Failed: gunzip tag_1_pythia8_events.hepmc.gz" && rsync -ah --info=progress2 tag_1_pythia8_events.hepmc $hepmc_path/$(basename "${d}").hepmc)
done
