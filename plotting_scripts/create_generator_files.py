# -*- coding: utf-8 -*-
"""
MG Generator Script Creator
______________________________________________________________________________
Script to automatically produce MadGraph generation script files
...
class Generator_Script:
    ...
    def add_lines_to_file(self, mass_array=None, csv_file=None, MN46_same=False, N46_mass=10, limit=0, codes={"CalcWidth":1, "HiggsExcl":1, "JetDef":1}): 
...
if __name__ == "__main__":
    ...
    top_lines_jet_def2= ["import model MLRSM",
                         "define bsmhiggs h h2 h02 h03 h+ hp2 hl++ hr++ h3 a02 h- hm2 hl-- hr--",
                         "define j = b b~",
                         "generate p p > mu+ mu+ j j / bsmhiggs w+ w- @1",
                         "add process p p > mu- mu- j j / bsmhiggs w+ w- @2"]
    
    top_lines_jet_def3= ["import model MLRSM",
                         "define bsmhiggs h h2 h02 h03 h+ hp2 hl++ hr++ h3 a02 h- hm2 hl-- hr--",
                         "define j = t t~",
                         "generate p p > mu+ mu+ j j / bsmhiggs w+ w- @1",
                         "add process p p > mu- mu- j j / bsmhiggs w+ w- @2"]
    ...
    jet_def2_gen_script = Generator_Script("JetDef2", top_lines_jet_def2, ft.path_convert("../generator_scripts/JetDef2_gen_script.txt"))
    jet_def2_gen_script.add_lines_to_file(mass_array=jet_test_points, MN46_same=True, codes={"CalcWidth":1, "HiggsExcl":1, "JetDef":2})
    jet_def2_gen_script.write_to_file()
    
    jet_def3_gen_script = Generator_Script("JetDef3", top_lines_jet_def3, ft.path_convert("../generator_scripts/JetDef3_gen_script.txt"))
    jet_def3_gen_script.add_lines_to_file(mass_array=jet_test_points, MN46_same=True, codes={"CalcWidth":1, "HiggsExcl":1, "JetDef":3})
    jet_def3_gen_script.write_to_file()
    
    jet_def4_gen_script = Generator_Script("JetDef4", top_lines_jet_def3, ft.path_convert("../generator_scripts/JetDef4_gen_script.txt"))
    jet_def4_gen_script.add_lines_to_file(mass_array=jet_test_points, MN46_same=True, codes={"CalcWidth":1, "HiggsExcl":1, "JetDef":4})
    jet_def4_gen_script.write_to_file()
    ...
    Codes:
        ...
        Jet definition- 0:MG default  1:Inc bbar, ttbar 2:just b 3: just t, 4: ttbar & bbar
